...
const works: Record<string, WorkData> = {
  ...
  "welfare-matching": {
    ...
    client: "非公開（福祉・介護事業者）",
    description:
      "福祉サービスを必要とする利用者・ご家族と、対応可能な事業者・専門職を最適にマッチングするWebアプリ。Bubbleを用いたノーコード開発で、要件確定から本番リリースまでを短期間・低コストで実現しました。条件入力 → 候補一覧 → 詳細閲覧 → 問い合わせ までを1サービスで完結。",
    challenge:
      "福祉領域は「地域 × サービス種別 × 対応可否 × 空き状況」が複雑に絡み、利用者側は電話で1件ずつ確認するしかなく負担が大きい。事業者側も問い合わせ対応に追われ、本来のケア業務を圧迫していました。スピード重視で改善検証を回せる仕組みが必要でした。",
    solution:
      "Bubbleでフロント／管理画面／DB／ワークフローを一括構築。利用者は条件を入力するだけで該当事業者の一覧を取得でき、各事業者の対応領域・空き状況・連絡フォームを1画面で確認可能。事業者側にも案件管理ダッシュボードを提供し、応答状況の見える化を実現。プロトタイプから本番までを通常開発の数分の一の工数でリリースしました。",
    results: [
      "Bubbleで要件定義→本番リリースまでを短期間で達成",
      "利用者の事業者探しを「電話複数件→1サービス内検索」に置換",
      "事業者側ダッシュボードで問い合わせ管理を一元化",
      "ノーコード基盤で運用中の改善サイクルも高速",
    ],
    techStack: ["Bubble", "ノーコード", "外部API連携", "レスポンシブUI"],
    image: "/works-auto-accounting.png",
    imageAlt: "福祉マッチングアプリのイメージ",
  },
  "ec-site": {
    title: "ECサイト構築",
    category: "EC Site",
