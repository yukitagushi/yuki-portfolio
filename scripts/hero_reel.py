...
def render_reel(videos=None):
    ...
    for index, item in enumerate(videos):
        ...
        first = index == 0
        slides.append(f'''<div class="reel-slide{' is-portrait' if height > width else ''}{' is-active' if first else ''}">
...
<video muted playsinline preload="none" data-src="{src}" poster="{poster}" width="{width}" height="{height}" tabindex="-1" disablepictureinpicture disableremoteplayback></video></div>''')
    ...
    return f'''<div class="hero-reel" data-hero-reel>
<div class="reel-slides" aria-hidden="true">{''.join(slides)}</div>
<div class="reel-overlay"><p class="reel-eyebrow">SELECTED WORKS / 制作実績</p><div class="reel-headline"><h1><span>つくるを、</span><span>もっと自由に。</span></h1><p>AI広告動画の制作と、仕事の自動化。</p></div></div>
