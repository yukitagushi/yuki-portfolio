...
def head(title, description, path, schemas=(), article=False, noindex=False, og_image=None, og_image_alt=None, og_image_width=1200, og_image_height=630, stylesheets=(), scripts=()):
    url = BASE + path
    image = og_image or BASE + '/uploads/ogp_ai_portfolio.jpg'
    if image.startswith('/'):
        image = BASE + image
    image_alt = og_image_alt or 'Yuki Taguchi — AI Video & Automation'
...
<meta property="og:type" content="{'article' if article else 'website'}"><meta property="og:title" content="{esc(title)} | Yuki Taguchi"><meta property="og:description" content="{esc(description)}"><meta property="og:url" content="{url}"><meta property="og:image" content="{image}"><meta property="og:image:width" content="{int(og_image_width)}"><meta property="og:image:height" content="{int(og_image_height)}"><meta property="og:image:alt" content="{esc(image_alt)}"><meta property="og:locale" content="ja_JP"><meta property="og:site_name" content="Yuki Taguchi">
...
def page(title, description, path, body, schemas=(), article=False, noindex=False, og_image=None, og_image_alt=None, og_image_width=1200, og_image_height=630, stylesheets=(), scripts=()):
    overlay = path == '/' and 'data-hero-reel' in body
    body_class = 'refresh home-page' if overlay else 'refresh'
    return head(title, description, path, schemas, article, noindex, og_image, og_image_alt, og_image_width, og_image_height, stylesheets, scripts) + f'<body class="{body_class}">' + header(overlay) + '<main id="main">' + body + '</main>' + footer() + '</body></html>'
